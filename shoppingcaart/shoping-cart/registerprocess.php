...
if (isset($_POST['submit'])) {
  ...
  // We check for an invalid username AND invalid e-mail.
    ...
    header("Location: login.php?error=invaliddmail");
    exit();
  }
  // We check for an invalid e-mail.
  else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    header("Location: login.php?error=invalidmail&uid=");
    exit();
  }
  // We check if the repeated password is NOT the same.
  else if ($password !== $passwordRepeat) {
    header("Location: login.php?error=passwordcheck&uid=".$email);
    exit();
  }
  else {
    ...
    $sql = "SELECT email_client FROM client WHERE email_client=?;";
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      if ($resultCount > 0) {
        ...
        $sql = "INSERT INTO client (email_client, password_client) VALUES (?, ?);";
