...
//   update query
$up_sql = "UPDATE client SET prenom_client='$fname', nom_client='$lname',  company='$companyName', address1='$addr1', address2='$addr2', city='$city', country='$country', zip='$Postcode', mobile='$Phone'  WHERE id_client=$cid";
...
if($Updated){
	...
	$insertOrder = "INSERT INTO commande (id_client, total_commande, etat, paymentmode )
	...
	if(mysqli_query($conn, $insertOrder)){
		...
		foreach($cart as $key => $value){ 
		   ...
		   $result_cart = mysqli_query($conn, $sql_cart);
		   $row_cart = mysqli_fetch_assoc($result_cart); 
			$price_product = $row_cart["prix"];
			 $q  = $value["quantity"];
		   $insertordersItems = "INSERT INTO lignecommande (id_commande, id_article, quantite, productprice) 
  ...
  $ins_sql = "INSERT INTO client (id_client,prenom_client,nom_client, company, address1, address2, city, country, zip, mobile)
