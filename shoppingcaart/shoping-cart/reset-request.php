...
if (isset($_POST['reset-request-submit'])) {
  ...
  $url = "http://akin.company/hielo-772466825/shoppingcaart/shoping-cart/create-new-password.php?selector=" . $selector . "&validator=" . bin2hex($token);
  ...
  $sql = "DELETE FROM pwdreset WHERE reset_email=?";
  ...
  $sql = "INSERT INTO pwdreset (reset_email, reset_selector, reset_token, reset_expires) VALUES (?, ?, ?, ?)";
  $stmt = mysqli_stmt_init($conn);
  if (!mysqli_stmt_prepare($stmt, $sql)) {
    echo "There was an error!";
    exit();
  ...
  // Subject
  $subject = 'Reset your password for akin';
  ...
  // Headers
  // Headers
  $headers = "From: noreply@akin.company>\r\n";
  $headers .= "Reply-To: noreply@akin.company\r\n";
